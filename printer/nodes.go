	...
	if fields != nil {
		...
		default:
			...
			if fields.List[0] != nil {
				...
				p.print(token.NoPos, token.RPAREN)
				p.print(blank)
			}
		}
	}
...
func (p *printer) parameters0(open token.Pos, fields []*ast.Field, close token.Pos) {
	if len(fields) != 0 {
		prevLine := p.lineFor(open)
		ws := indent
		for i, par := range fields {
			if par == nil || par.Type == nil {
				continue
			}
		...
		if closing := p.lineFor(close); 0 < prevLine && prevLine < closing {
			p.print(token.COMMA)
